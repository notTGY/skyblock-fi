    ...
    if (active && payload && payload.length) {
      return (
        <div className="bg-card border border-border p-2 text-[10px] font-mono">
          ...
          <div className="text-chart-3">Buy Vol: {payload[0].value.toLocaleString()}</div>
          <div className="text-chart-5">Sell Vol: {payload[1].value.toLocaleString()}</div>
        </div>
      )
    }
    return null
    ...
    <Card className="p-3">
      ...
      <ResponsiveContainer width="100%" height={150}>
        <BarChart data={chartData} margin={{ top: 5, right: 5, left: 0, bottom: 0 }}>
          ...
          <Bar dataKey="buyVol" fill="hsl(var(--chart-3))" radius={[2, 2, 0, 0]} />
          <Bar dataKey="sellVol" fill="hsl(var(--chart-5))" radius={[2, 2, 0, 0]} />
        </BarChart>
      </ResponsiveContainer>
